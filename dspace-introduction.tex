...
\begin{frame}{Basemaps II}
 \begin{columns}
  \column{.5\textwidth}
   \includegraphics[scale=0.5]{basemap_tile}
  \column{.5\textwidth}
   \begin{itemize}
...
\begin{frame}{Overlays}
 \begin{columns}
  \column{.5\textwidth}
   ...
   \end{itemize}
  \column{.5\textwidth}
   \includegraphics[scale=0.5]{overlay_bar}
 \end{columns}
\end{frame}
...
\begin{frame}{Overlays}
 \begin{columns}
  \column{.5\textwidth}
   ...
   \end{itemize}
  \column{.5\textwidth}
   \includegraphics[scale=0.5]{overlay_foo}
 \end{columns}
\end{frame}
...
\begin{frame}{OSM}
 We love the OSM project and fundamental for DSpace, but:
 \begin{columns}
  \column{.5\textwidth}
    {\small Not everything is in OSM:
      \begin{itemize}
       \item Topo
       \item Aerial
      \end{itemize}
  ...
  \column{.5\textwidth}
    {\small Not everything belongs in OSM
      \begin{itemize}
       \item Tracking
       \item Personal/Private/Group relad POIs
      \end{itemize}
    }
    \includegraphics[scale=0.169]{map_render_dspace}
 \end{columns}
-> openSTREETMap
%    user collects data -> OSM -> OSM team filters/manages additions -> OSM Updates releases
%    -> OSM Dump -> PostGIS -> TileMill + SRTM Topo Data -> TileStream -> user receives/updates data
\end{frame}
